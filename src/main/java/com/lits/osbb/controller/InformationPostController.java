...
public class InformationPostController {
    ...
    ResponseEntity<InformationPostRepository> deleteOneById(@PathVariable("id") Long id) {
    ...
    ResponseEntity<?> saveAffiche(@RequestParam(value = "informationPostDto") InformationPostDto informationPostDto) {
        return new ResponseEntity<ResponseEntity<?>>(informationPostService.save(informationPostDto), HttpStatus.OK);
    }
